	...
	params := protocol.SendTaskParams{
		...
		Message: protocol.Message{
			Message: &v1.Message{
				Role: protocol.MessageRoleUser,
				Content: []*protocol.Part{
					{Part: &v1.Part_Text{Text: "Client SSE test"}},
				},
			},
		},
	}
...
func TestA2AClient_SetPushNotification(t *testing.T) {
	...
	t.Run("SetPushNotification Success", func(t *testing.T) {
		...
		mockHandler := createMockServerHandler(
			...
			nil, // Skip request validation for now due to protobuf complexity
			respBody,
			http.StatusOK,
			nil,
		)
		...
		assert.Equal(t, taskID, result.Name)
